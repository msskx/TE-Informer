...
import os
import torch
#  https://blog.csdn.net/qq_20144897/article/details/127314218
...
# 数据根目录
parser.add_argument('--root_path', type=str, default='./data/AQI/', help='root path of the data file')
# 数据文件名
parser.add_argument('--data_path', type=str, default='AQI.csv', help='data file')
# 预测任务，选项：[M，S，MS]；M：多变量预测多变量，S：单变量预测单变量，MS：多变量预测单变量。
parser.add_argument('--features', type=str, default='MS',
                    help='forecasting task, options:[M, S, MS]; M:multivariate predict multivariate, S:univariate predict univariate, MS:multivariate predict univariate')
parser.add_argument('--target', type=str, default='AQI', help='target feature in S or MS task')
# 时间特征编码的频率，选项：[s：每秒，t：每分钟，h：每小时，d：每日，b：工作日，w：每周，m：每月]，您还可以使用更详细的频率，比如15分钟或3小时。
...
data_parser = {
    ...
    'AQI': {'data': 'AQI.csv', 'T': 'AQI', 'M': [7, 7, 7], 'S': [1, 1, 1], 'MS': [7, 7, 1]},
    'TEAQI': {'data': 'TEAQI.csv', 'T': 'AQI', 'M': [11, 11, 11], 'S': [1, 1, 1], 'MS': [11, 11, 1]},
}
